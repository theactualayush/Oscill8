...
applied to the RIC): CORRA/SONIA must route to QuantHub, everything
else (e.g. SOFR) must keep using LSEG exactly as before, and a market
routed to LSEG must remain fully usable with no QuantHub credentials
configured at all.
...
def test_corra_routes_to_quanthub_not_lseg(mocker):
...
def test_sonia_routes_to_quanthub_not_lseg(mocker):
    mock_qh = mocker.patch(
        "database.service.download_history_quanthub",
    ...
    service.get_history(_SONIA_RIC, "DAILY", "2026-01-01", "2026-01-01")
...
def test_quanthub_routed_market_raises_clear_error_with_no_credentials(mocker, monkeypatch):
    monkeypatch.setattr(config, "QUANTHUB_TOKEN", "")
